<?php
/**
 * Data-extraction layer for MMDVMHost mode.
 *
 * The heart of the dashboard's mode-aware rendering. ~1250 lines of
 * procedural helpers consumed by /index.php, /mmdvmhost/repeaterinfo.php,
 * /mmdvmhost/lh.php, /mmdvmhost/localtx.php, /api/last_heard.php, and
 * the protocol-specific partials.
 *
 * Big picture, in three layers:
 *
 *   1. Config readers — slurp `/etc/mmdvmhost`, `/etc/ysfgateway`,
 *      `/etc/p25gateway`, `/etc/nxdngateway`, `/etc/m17gateway`,
 *      `/etc/dapnetgateway` into line arrays. {@see getConfigItem()} is
 *      the workhorse: find the value of a key inside a `[Section]`.
 *
 *   2. Service / login / mode-status — {@see isProcessRunning()} (in
 *      tools.php), {@see checkDMRLogin()}, {@see showMode()} which emits
 *      the coloured mode grid via a giant per-protocol switch.
 *
 *   3. Log parsers — {@see getMMDVMLog()} and friends shell out to
 *      `egrep | sed | tail` against today's daily MMDVM/YSF/P25/NXDN/M17/
 *      DAPNET log under /var/log/pi-star/, with a fallback to yesterday's
 *      log when today is empty (gmdate-driven, with a 60 s clock-skew
 *      buffer via `time() - 86340`). {@see getHeardList()} parses the
 *      MMDVM log into a structured 10-tuple per call (timestamp, mode,
 *      callsign, id, target, source, duration, loss, BER, RSSI), and
 *      {@see getLastHeard()} dedupes that into the visible feed used
 *      by lh.php and the JSON API.
 *
 * IMPORTANT — many functions in here use **hardcoded substr offsets**
 * against log file lines (e.g. `substr($logLine, 27, …)`). Sample log
 * lines are documented inline as comments next to each parser. If
 * upstream gateway log formats drift those offsets break silently, so
 * preserve the example log lines verbatim and validate against a
 * running device when modifying any parser.
 *
 * No setSecurityHeaders() call here — this file is `include_once`'d
